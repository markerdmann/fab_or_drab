...
- (id)initWithTweet:(Twitter *)twt
{
  if ( self = [super initWithStyle:UITableViewStyleGrouped] )
	{
		self.tweet = twt;
		[self setGrayNavigationBarWithTitle:@"Retweet"];
		[self setGrayNavBarBackButton];
	}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
	...
	{	
		...
		imageView.image					= [UIImage imageNamed:@"icon.jpg" scaledToSize:imageSize];
...
{	
	if ( indexPath.section == 0 )
	{
		...
		dateLabel.font		  = [UIFont fontWithName:@"Verdana" size:12.0f];
